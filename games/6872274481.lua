...
run(function()
	...
	local function safeRemoveTags(text)
		...
		if typeof(removeTags) ~= 'function' then return text end
		local ok, result = pcall(removeTags, text)
		return (ok and typeof(result) == 'string') and result or text
	end

	local function safeGetFontSize(text, textSize, font, bounds)
		if typeof(getfontsize) ~= 'function' then return Vector2.new(120, textSize + 8) end
		local ok, size = pcall(getfontsize, text, textSize, font, bounds)
		return (ok and typeof(size) == 'Vector2') and size or Vector2.new(#text * textSize * 0.6, textSize + 8)
	end

	local function getFallbackColor()
		if Color and typeof(Color.Hue) == 'number' then
	...
	local function safeGetEntityColor(ent)
		local c
		pcall(function() c = entitylib.getEntityColor(ent) end)
		return c
	end
	...
	local function getDistance(ent)
		...
		if localRoot and ent and ent.RootPart then
			...
			if ok and typeof(dist) == 'number' then return dist end
		end
		return math.huge
	end
	...
	local function getSafeDistance(ent)
		...
		if dist == math.huge or dist ~= dist then return 0 end
		return math.floor(dist)
	end

	local function getDistancePrefixRich(mag)
		return '<font color="rgb(85,255,85)">[</font><font color="rgb(255,255,255)">' .. tostring(mag) .. '</font><font color="rgb(85,255,85)">]</font> '
	end
	...
	local function buildBaseName(ent, richTags)
		if ent and ent.Player then
			local tag = ''
			pcall(function() tag = whitelist:tag(ent.Player, true, richTags and true or false) end)
			if typeof(tag) ~= 'string' then tag = '' end
		...
		pcall(function()
			if ent and ent.Character and ent.Character.Name then charName = ent.Character.Name end
		end)
		return charName
	end

	local function appendHealthTextNormal(base, ent)
		if not Health or not Health.Enabled then return base end
		local current = tonumber(ent.Health) or 0
		local max = tonumber(ent.MaxHealth) or 100
		if max <= 0 then max = 100 end
		local ratio = math.clamp(current / max, 0, 1)
		local hColor = Color3.fromHSV(ratio / 2.5, 0.89, 0.75)
		return base .. ' <font color="rgb(' .. math.floor(hColor.R * 255) .. ',' .. math.floor(hColor.G * 255) .. ',' .. math.floor(hColor.B * 255) .. ')">' .. math.round(current) .. '</font>'
	end
	...
	local function shouldSkipEntity(ent)
		...
		if Targets then
			if ent.Player and Targets.Players and not Targets.Players.Enabled then return true end
			if ent.NPC and Targets.NPCs and not Targets.NPCs.Enabled then return true end
		end
		if Teammates and Teammates.Enabled and (not ent.Targetable) and (not ent.Friend) then return true end
		return false
	end
	...
	local okRemotes, remotesModule = pcall(function() return require(rankinfo.ReplicatedStorage.TS.remotes).default end)
	rankinfo.Client = (okRemotes and remotesModule) or bedwars.Client
	local okRankMeta, rankMetaModule = pcall(function() return require(rankinfo.ReplicatedStorage.TS.rank['rank-meta']).RankMeta end)
	...
	function rankinfo:RequestRank(player)
		...
		if self.Cache[userId] or self.Pending[userId] then return end
		local lastFail = self.Failed[userId]
		if lastFail and tick() - lastFail < 10 then return end
	...
	function rankinfo:GetImage(player)
		...
		if rank and self.RankMeta[rank] then return self.RankMeta[rank].image end
	...
	function refreshAllNametags()
		...
		for ent in Reference do pcall(refreshNametag, ent) end
	end
	...
	local function normalizeItem(item)
		local t = typeof(item)
		if t == 'string' then return { itemType = item } end
		if t == 'table' then return item end
		return nil
	end

	local function getItemTypeName(item)
		item = normalizeItem(item)
		if item and typeof(item.itemType) == 'string' and item.itemType ~= '' then return item.itemType end
		return nil
	end

	-- [修正] getIconのエラーハンドリング追加
	local function safeGetIcon(item)
		item = normalizeItem(item)
		if not item or not getItemTypeName(item) then return '' end
		if not bedwars or typeof(bedwars.getIcon) ~= 'function' then return '' end
		local ok, icon = pcall(function() return bedwars.getIcon(item, true) end)
	...
	local function getArmorItem(inv, index, altNames)
		...
		if typeof(armor) ~= 'table' then return nil end
		
		-- 標準インデックスで取得
		local item = armor[index]
		if item then return item end
		
		-- [修正] 環境差対策: 4,5,6 で取れなければ 1,2,3 を試す
		if index >= 4 then
			item = armor[index - 3]
			if item then return item end
		end
	...
	local function safeCountPlayerResources(plr)
		...
		if ok and typeof(counts) == 'table' then
			for _, res in RESOURCE_CONFIG do counts[res.name] = tonumber(counts[res.name]) or 0 end
			return counts
		end
	...
	local function removeResourceIcons(nametag)
		...
		for _, child in nametag:GetChildren() do
			if child.Name:match('^ResIcon_') or child.Name:match('^ResText_') then child:Destroy() end
		end
	end
	...
	local function getResourceDrawingText(ent)
		...
		local parts = {}
		for _, res in RESOURCE_CONFIG do
			if counts[res.name] > 0 then table.insert(parts, string.format('%s:%d', res.key:sub(1, 2):upper(), counts[res.name])) end
		end
	...
	local function getKitMeta(player)
		...
		if player then
			pcall(function()
				...
				if typeof(attr) == 'string' and attr ~= '' then kit = attr end
			end)
		end
		local meta
		pcall(function()
	...
	local function ensureEquipmentIcons(nametag)
		if not nametag or typeof(nametag.FindFirstChild) ~= 'function' then return end
	...
	local function setEquipmentImage(nametag, slot, image)
		if not nametag or typeof(nametag.FindFirstChild) ~= 'function' then return end
		local icon = nametag:FindFirstChild(slot)
		if icon then icon.Image = image or '' end
	end

	-- [修正] Equipment更新処理の安定化
	...
	local function getEquipmentText(ent)
		if not Equipment or not Equipment.Enabled or not ent or not ent.Player then return '' end
		local inv = getInventory(ent.Player)
		local parts = {}
		local function addPart(item)
			local itemName = getItemTypeName(item)
			if itemName then table.insert(parts, itemName) end
		end
	...
	-- ランク / デバイス ヘルパー (既存維持)
	-- =========================================================
	local DEVICE_EMOJI = { PC = '💻', MOBILE = '📱', Console = '🎮' }
	...
	local function updateDeviceLabel(ent, nametag)
		...
		local device
		pcall(function() device = ent.Player:GetAttribute('UserInputType') end)
		if typeof(device) ~= 'string' or device == '' then device = '???' end
	...
	local function getRankText(ent)
		if not RankDisplay or not RankDisplay.Enabled or not ent or not ent.Player then return '' end
		local rank = rankinfo:GetRank(ent.Player)
		if not rank then rankinfo:RequestRank(ent.Player); return '' end
		local meta = rankinfo.RankMeta[rank]
		return meta and (' {' .. tostring(meta.name or rank) .. '}') or ''
	end

	local function getDeviceText(ent)
		if not DisplayDevice or not DisplayDevice.Enabled or not ent or not ent.Player then return '' end
		local device
		pcall(function() device = ent.Player:GetAttribute('UserInputType') end)
		if typeof(device) ~= 'string' or device == '' then device = '???' end
		return ' [' .. device .. ']'
	...
	local Added = {
		Normal = function(ent)
			if shouldSkipEntity(ent) then return end
	...
	refreshNametag = function(ent)
		if methodused and Updated[methodused] then pcall(Updated[methodused], ent) end
	end

	local ColorFunc = {
		Normal = function(h, s, v)
			local c = Color3.fromHSV(h, s, v)
			for i, obj in Reference do pcall(function() obj.TextColor3 = safeGetEntityColor(i) or c end) end
		end,
		Drawing = function(h, s, v)
			local c = Color3.fromHSV(h, s, v)
			for i, obj in Reference do pcall(function() if obj.Text then obj.Text.Color = safeGetEntityColor(i) or c end end) end
		end,
	}
	...
	local Loop = {
			...
			for ent, nametag in Reference do
				...
				if ent.Player and ((Equipment and Equipment.Enabled) or (DisplayResource and DisplayResource.Enabled)) then
					if not ent.LastEquipUpdate or now - ent.LastEquipUpdate >= EQUIPMENT_UPDATE_INTERVAL then
						...
						if Equipment and Equipment.Enabled then pcall(updateEquipmentIcons, ent, nametag) end
						if DisplayResource and DisplayResource.Enabled then
							local size = safeGetFontSize(safeRemoveTags(nametag.Text), nametag.TextSize, nametag.FontFace, Vector2.new(100000, 100000))
							pcall(updateResourceIcons, ent, nametag, size.X + 8)
						end
					end
				end
				...
				local okView = pcall(function()
					...
					headPos, headVis = gameCamera:WorldToViewportPoint(ent.RootPart.Position + Vector3.new(0, hipHeight + 1, 0))
				end)
				...
				if Distance and Distance.Enabled then
					...
					if Sizes[ent] ~= mag then
						...
						nametag.Size = UDim2.fromOffset(size.X + 8, size.Y + 7)
						Sizes[ent] = mag
						if DisplayResource and DisplayResource.Enabled then pcall(updateResourceIcons, ent, nametag, size.X + 8) end
					end
				end
		...
		Drawing = function()
			for ent, nametag in Reference do
				...
				if not okView or not headPos then nametag.Text.Visible = false; nametag.BG.Visible = false; continue end
			...
			if callback then
				...
				if entitylib and entitylib.Events then
					if Removed[methodused] and entitylib.Events.EntityRemoved then
						NameTags:Clean(entitylib.Events.EntityRemoved:Connect(function(ent) pcall(Removed[methodused], ent) end))
					end
					if Added[methodused] then
						if entitylib.List then
							for _, v in entitylib.List do
								if Reference[v] then pcall(Removed[methodused], v) end
								pcall(Added[methodused], v)
							end
						end
						if entitylib.Events.EntityAdded then
							NameTags:Clean(entitylib.Events.EntityAdded:Connect(function(ent)
								if Reference[ent] then pcall(Removed[methodused], ent) end
					...
					if Updated[methodused] then
						if entitylib.Events.EntityUpdated then
							NameTags:Clean(entitylib.Events.EntityUpdated:Connect(function(ent) pcall(Updated[methodused], ent) end))
						end
						if entitylib.List then for _, v in entitylib.List do pcall(Updated[methodused], v) end end
					end
				end
				if ColorFunc[methodused] and vape.Categories.Friends and vape.Categories.Friends.ColorUpdate then
					NameTags:Clean(vape.Categories.Friends.ColorUpdate.Event:Connect(function()
						if Color then pcall(ColorFunc[methodused], Color.Hue, Color.Sat, Color.Value) end
					end))
				end
				if Loop[methodused] and runService and runService.RenderStepped then
					NameTags:Clean(runService.RenderStepped:Connect(function() pcall(Loop[methodused]) end))
				end
			else
				if Removed[methodused] then for i in Reference do pcall(Removed[methodused], i) end end
			end
		end,
	...
	RankDisplay = NameTags:CreateToggle({ Name = 'Rank Display', Tooltip = 'Shows the player rank icon next to the name', Function = function() if NameTags.Enabled then NameTags:Toggle(); NameTags:Toggle() end end })
	DisplayDevice = NameTags:CreateToggle({ Name = 'Display Device', Tooltip = 'Shows the player device as an emoji (PC/Mobile/Console)', Function = function() if NameTags.Enabled then NameTags:Toggle(); NameTags:Toggle() end end })
	DisplayResource = NameTags:CreateToggle({ Name = 'Display Resource', Tooltip = 'Shows Iron, Gold, Diamond, Emerald with icons next to names', Function = function() if NameTags.Enabled then NameTags:Toggle(); NameTags:Toggle() end end })
	...
	DistanceCheck = NameTags:CreateToggle({ Name = 'Distance Check', Function = function(cb) if DistanceLimit and DistanceLimit.Object then DistanceLimit.Object.Visible = cb end end })
