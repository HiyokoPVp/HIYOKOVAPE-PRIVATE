...
run(function()
    local RocketCrasher
    local zap = replicatedStorage:FindFirstChild("ZAP", true) and replicatedStorage.ZAP:FindFirstChild("ZAP_RELIABLE")
    local bufferData
    
    -- Base64デコード関数（提供されたコードベース）
    local function decodeBase64(str)
        local raw = httpService:Base64Decode(str)
        local buf = buffer.create(#raw)
        for i = 1, #raw do
            buffer.writeu8(buf, i - 1, string.byte(raw, i))
        end
        return buf
    end
    ...
    -- バッファデータの初期化
    pcall(function()
        bufferData = decodeBase64("AgHnN0FC/f/DQRm5WkNTODc0OUlXOQEAAQEBtmAjPgFDN0FZMlNMVQAAAA==")
    end)

    RocketCrasher = vape.Categories.Blatant:CreateModule({
        Name = 'RocketCrasher',
        Function = function(callback)
            if callback then
                if not zap or not bufferData then
                    notif('RocketCrasher', 'Failed to initialize ZAP or Buffer data.', 5, 'alert')
                    RocketCrasher:Toggle()
                    return
                end

                task.spawn(function()
                    while RocketCrasher.Enabled do
                        local char = lplr.Character
                        if char and char:FindFirstChild("HumanoidRootPart") then
                            local pos = char.HumanoidRootPart.Position
                            local posStr = string.format("%f, %f, %f", pos.X, pos.Y, pos.Z)
                            ...
                            local args = {
                                ...
                                {
                                    posStr,
                                    "Workspace.rocket_launcher_missile",
                                    "ReplicatedStorage.Inventories." .. lplr.Name .. ".rocket_launcher",
                                    "Workspace." .. lplr.Name
                                }
                            }
                            
                            pcall(function()
                                zap:FireServer(unpack(args))
                            end)
                        end
                        task.wait(0.01)
                    end
        ...
        Tooltip = 'Spams rocket launcher packets via ZAP reliable.'
    })
end)