...
local function getPosition(ent, proj)
    ...
    return ent[TargetPart.Value] and ent[TargetPart.Value].Position or ent.PrimaryPart.Position
end
        ...
        if callback then
            ...
            launchHook = bedwars.ProjectileLaunchHook:Add('ProjectileAimbot', 100, function(nextLaunch, ...)
                ...
                if plr then
                    ...
                    -- 予測計算 (PredictionX, PredictionY を引数として渡す)
                    local calc = prediction.SolveTrajectory(
                        newlook.p, 
                        projSpeed, 
                        gravity, 
                        targetpos, 
                        projmeta.projectile == 'telepearl' and Vector3.zero or v, 
                        playerGravity, 
                        plr.HipHeight, 
                        plr.Jumping and 42.6 or nil, 
                        rayCheck,
                        PredictionX.Value, -- X (横) の予測倍率
                        PredictionY.Value  -- Y (上下) の予測倍率
                    )
